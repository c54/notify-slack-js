...
// target service 
var service_info_json = config.service_properties.connpass;
...
function notify() {
	...
	function notify_slack(json_text) {
		...
		for (var i=0; i<json_obj.results_returned; i++) {
			var _title = json_obj.events[i].title;
			var _owner = json_obj.events[i].owner_nickname;
			var _time = json_obj.events[i].started_at;
			var _address = json_obj.events[i].address;
			var _place = json_obj.events[i].place;
			var _url = json_obj.events[i].event_url;
			var _hash_tag = json_obj.events[i].hash_tag;
			var _limit = json_obj.events[i].limit;
			var _accepted = json_obj.events[i].accepted;
